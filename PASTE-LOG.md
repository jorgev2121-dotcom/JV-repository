...
| PASTE-D-024 | 2026-08-24 | Desktop — live executor session | **Set the Desktop's window label (RI-031).** Add a statusline to the Desktop's **user** settings (`~/.claude/settings.json`, NOT the shared repo `.claude/settings.json` — that would mislabel the cloud too) so it shows `🖥️ CODE · DESKTOP EXECUTOR` on every turn: `{"statusLine":{"type":"command","command":"echo '🖥️ CODE · DESKTOP EXECUTOR'"}}`. From then on open every session with that banner and prefix every reply with 🖥️ (charter §10 item 1). Confirm the statusline shows, via TO-CLOUD.md. | ISSUED |
