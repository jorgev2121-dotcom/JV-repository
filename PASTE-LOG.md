...
- **PASTE-X-007** · 2026-09-19 · Cloud → Cowork (🤝) → click "Finish setup" on CData Connect AI, Google Calendar, Google Drive in the ChatGPT desktop Plugins page; sign in as Jorge at each login prompt (Chrome, his default browser)
- **PASTE-X-008** · 2026-09-19 · Cloud → Cowork (🤝) → re-authenticate the expired ChatGPT desktop plugins (whichever now show "reconnect"/expired, not just unfinished setup); sign in as Jorge at each prompt (Chrome)
- **PASTE-X-009** · 2026-09-19 · Cloud → Cowork (🤝) → sign in to the 14 Codex-lane connectors once RAMBO has installed them: Box, Codex Security, Figma, GitHub, Google Calendar, Google Drive, Linear, Notion, OpenAI Developers, Outlook Calendar, Outlook Email, SharePoint, Slack, Teams — one at a time, Jorge completes any password/2FA step
- **PASTE-X-010** · 2026-09-19 · Cloud → Cowork (🤝) → Desktop cleanup, SURVEY-ONLY phase: inventory every real icon on the visible Desktop, OCR each file, assign a provisional OPH number to anything untracked. Do NOT move, rename, delete, or refile anything — empties get listed, not touched. Report back as one file for Jorge's yes before any move happens.
- **PASTE-X-011** · 2026-09-19 · Cloud → Cowork (🤝) → status check, not new work: report how many of the 14 connectors (item 19/PASTE-X-009 — WORK-QUEUE item renumbered 2026-09-23, was item 16) are signed in, and how the Desktop survey (PASTE-X-010) is progressing — a count, not "working on it"
