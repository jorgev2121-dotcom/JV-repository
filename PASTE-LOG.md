...
- **PASTE-D-031** · 2026-08-25 · Desktop → SUPERSEDES the guessing script. Copy the KNOWN tax-jacket test-set folder (the one desktop has been OCR-testing) to Desktop\Tray 6, copy-only; do NOT guess by most-files; do NOT recurse all of OneDrive; detect tools via magick.exe (not convert.exe). Report count + tool table.
- **PASTE-X-004** · 2026-08-25 · ChatGPT → tax-jacket enhancement prompt (Python sandbox: PyMuPDF/OpenCV/Pillow; page-type-aware; guardrails; before/after montage). File: JACKET-ENHANCEMENT_CHATGPT-PROMPT.md
- **PASTE-D-032** · 2026-08-25 · Desktop → stage the 16-page test jacket (331 Tamiami Canal Rd pt1) into Trey 6 subfolder on the VISIBLE desktop (verify C:\Users\JV\OneDrive\Desktop vs C:\Users\JV\Desktop, use the one that shows on screen); COPY only; report full path. (ChatGPTs PASTE-D-036 was an invented number + hidden-desktop path.)
...
- **PASTE-D-034** · 2026-09-19 · Desktop (Cowork also authorized) → item 13 in `WORK-QUEUE.md` (OD-107 / RI-046): Word M365 passkey sign-in loop + 9Router "3 attempts left" are one fault — 1Password locked and not the Windows passkey provider. Unlock with Jorge, Hello ON, passkey provider ON, retry Word ("Sign in another way" as fallback), fix 9Router without guessing, save localhost logins with port in URL, disable the RI-018 PAD code-monitor. Cloud's first reply mis-called the screenshots as phishing; corrected the same session
- **PASTE-D-035** · 2026-09-19 · Desktop → supersedes D-034's pointer, same item 13 (OD-107) now with step 0: Outlook "exhausted all shared resources" ×3 = kill every zombie `OUTLOOK.EXE` + Teams/Skype, reopen, `/resetnavpane` if it returns; 9Router facts added (port 20128, `C:\Users\JV\.9router\db\data.sqlite`, `123456` only valid while no saved hash — clear the hash row, never the file; bind 127.0.0.1 only)
- **PASTE-X-006** · 2026-09-19 · COWORK (computer-use, on Jorge's PC) → execute WORK-QUEUE item 13 (OD-107) hands-on because the desktop executor has not answered D-034/D-035 and cloud has no device link (checked: no remote-device tools in this session — IMPOSSIBLE from cloud, not blocked). Order: Outlook zombie kill → 1Password unlock (Jorge touches Hello only) → passkey provider ON → Word "Sign in another way" fallback → 9Router `123456` by hand → report to TO-CLOUD.md
- **PASTE-D-037** · 2026-09-20 · Desktop (Cowork also authorized) → WORK-QUEUE item 14 (TRK-2026-9946): fetch the branch, checkout the staged JSON only, MERGE the owner-approved permissions into the live project `.claude\settings.json` (backup → union → UTF-8 no-BOM → re-parse → rollback on failure), then prove a fresh session runs `git status` with no prompt. Owner approval quoted verbatim inside the item; Jorge's "every reset links to 1Password" rule included. Cloud's own write was classifier-refused (RI-047) and not routed around.
- **PASTE-D-037 CONFIRMED** · 2026-09-20 · Desktop RAMBO cycle ~11:2x PM ET applied TRK-2026-9946 EXECUTED-WITH-PROOF (backup, 150/43/50 rules, acceptEdits, PARSE OK, model + SessionStart hook intact) via the Drive `VTES-Inbox` copy (git copy differed only by autocrlf). Step-4 "fresh session, no prompt" observation still owed.
- **PASTE-X-007** · 2026-09-20 · COWORK → do NOT create the bridge-reconnect scheduled task; the bridge is up. Read `.claude/skills/desktop-blocked-task/SKILL.md` on branch `claude/new-session-1j77e1` and follow it: GREEN steps go to RAMBO via `VTES-Inbox`, the RED step is one Windows Hello touch with Jorge present. Your only remaining OD-107 item is AP-107-07 (browser autofill competitors), to be done live with Jorge, not scheduled.
- **PASTE-D-038** · 2026-09-20 · Desktop → WORK-QUEUE item 15 (TRK-2026-9958): narrow-checkout the updated `vts-llm-panel/vts_llm_panel.py`, run `--health` (expect `ollama LIVE`) and one real prompt (expect `[answered by: ollama]`), paste output to TO-CLOUD.md; read-only report of the 9Router version + port binding (TRK-2026-9961). No install, no uninstall, no keys.
- **PASTE-D-039** · 2026-09-20 · Desktop (Cowork also authorized), delivered via Drive `VTES-Inbox` → OD-107 step 0 is reproducing now: detect the "Outlook Data File / exhausted all shared resources" dialog, kill OUTLOOK.EXE only on that proof, restart, `/resetnavpane` if it returns, report; print the one Hello-touch sentence if Jorge is present.
- **PASTE-D-040** · 2026-09-20 · Desktop → WORK-QUEUE item 16 (TRK-2026-9970): narrow-checkout `tools/filetree`, run `Scan-Tree.ps1` on OneDrive Documents and the local G: 01-JOBS mirror (no admin, metadata only), build the panel, paste the denominator lines, copy the two JSONs to VTES-Outbox. No TreeSize/WizTree (UAC).
- **PASTE-D-041** · 2026-09-20 · Desktop → WORK-QUEUE item 17 (TRK-2026-9961): stop the 9Router process on :20128, `npm uninstall -g 9router`, remove its config dir if present, confirm both checks empty, paste to TO-CLOUD.md. Owner-approved verbatim.
- **PASTE-D-043** · 2026-09-20 · Desktop (URGENT, Cowork also authorized) → WORK-QUEUE item 19: kill stray Outlook, stop WSearch, find+disable any scheduled task auto-launching Outlook, restart normal or Safe Mode. Owner pre-authorized every step.
- **PASTE-D-044** · 2026-09-20 · Desktop → WORK-QUEUE item 20 (TRK-2026-9995): restructure `morning_report.ps1`'s System Status block to loop over the same connector list the watchdog already checks (fixes Ollama/Grok being checked-but-hidden, and any future connector automatically); two small bugs flagged in passing.
- **PASTE-D-045** · 2026-09-20 · Desktop → WORK-QUEUE item 22 (TRK-2026-9999): confirm XAI_API_KEY state, test a fresh key directly if one now exists, confirm the VTS panel already picks it up, disable (not delete) the scheduled task(s) starting LiteLLM/VerticalTray. Explicitly does NOT repair LiteLLM — RI-038 already ruled that out twice.
- **PASTE-D-046** · 2026-09-20 · Desktop, live window only → catch-up pointer covering everything filed since this morning that the desktop's own unattended cycle may not yet have reached: `mailbox/to-desktop/WORK-QUEUE.md` items 17-22 (9Router removal, Outlook safe-mode/indexer, Filing Tree local scans, morning_report.ps1 fix, Grok/XAI diagnostic). Issued after Jorge reported no visible desktop-executor pop-up and asked directly where to paste.
- **PASTE-X-008** · 2026-09-20 · Anywhere (browser bookmark, not one specific window) → Claude Window Hub link (TRK-2026-10005): https://claude.ai/artifact/LybbUoCbLmaXoEdudTmML4 — open once, then pin per the page's own 3-step instructions so it survives as a taskbar icon.
- **PASTE-X-009** · 2026-09-20 · COWORK (Drive drop box) → four parallel workstreams retrieving permit document images for TRK-1292/1286/1535/1534, plus a fifth plain-language interpreter pass with general (non-official) effective-life estimates per work type. **STALLED — 2+ hours zero mailbox activity as of the 20:50 check, emailed Jorge.**
- **PASTE-D-047** · 2026-09-20 · Desktop → WORK-QUEUE item 23: confirm `VTES-Repo-Heartbeat`'s live state/interval (the real 3-minute task Jorge remembered), fold in TRK-2026-10002's CPU/process check. Read-only, no cadence change until the CPU finding is understood.
- **PASTE-D-048** · 2026-09-20 · Desktop → WORK-QUEUE item 24 (TRK-2026-10017): format the "Human IT Work Package" doc — title block (vendor fields TBD), book-style index, page numbers, footer stamp, hashtags. Vendor/account/appointment fields need Jorge's input before they can be filled for real.
- **PASTE-D-049** · 2026-09-20 · Desktop → WORK-QUEUE item 25 (TRK-2026-10017): insert the full "OWNER / JORGE REQUIRED" section (ranked owner-only tasks, verbatim text supplied) into the same Human IT Work Package document, after "Contents."
- **PASTE-D-050** · 2026-09-20 · Desktop → WORK-QUEUE item 26 (TRK-2026-10017): check whether the document already covers "the cloud server" (Jorge's question); if not, add the two-part cloud section (this Claude Code lane's own architecture + the still-unconfirmed DigitalOcean account, VAPOR finding JOB-0069).
- **PASTE-D-051** · 2026-09-20 · Desktop → WORK-QUEUE item 27 (TRK-2026-10017): add the existing shared folder-index structure (`_INDEX.html`/`_PORTAL_*.html`/`.SEARCH.txt`/`.TAGS.txt` + Filing Tree) to the dependency diagram and narrative report, with real file-timestamp dates, not an assumed "built yesterday."
- **PASTE-D-052** · 2026-09-20 · Desktop → WORK-QUEUE item 28 (URGENT): jobs 24-27 sat unpicked ~45 min — check RAMBO is alive (possible tie to TRK-2026-10002), run any not done, finalize the Human IT Work Package as one PDF, print to the default printer.
- **PASTE-D-053** · 2026-09-20 · Desktop → WORK-QUEUE item 29 (URGENT): Jorge relayed exit code -1073741510 (STATUS_CONTROL_C_EXIT — interrupted, not crashed). Confirm if this is RAMBO's own task, find the interrupt source, resume the queue from item 24.
- **PASTE-D-054** · 2026-09-21 · Desktop (Cowork also authorized) → WORK-QUEUE item 30 (TRK-2026-10052): get an actual saved copy (not a search summary) of the DBPR license page for CGC1528486 / MZ Solutions LLC, the Sunbiz entity page, the Miami-Dade C-number for 10980 SW 202 DR Unit 29 (folio 30-6007-009-0030), and screenshots of the reviewer comments. Owner-mandated: copies of the real pages only. Do not email MZ Solutions.
