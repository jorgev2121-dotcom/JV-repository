...
---
**RI-044 · 2026-08-27 — The owner supplies the recommendations the agents should have made.** Jorge, struggling with system navigation for months and saying so plainly, had to be the one to propose full agentic automation ("push agentic to the limit") — no agent, cloud included, volunteered it first. Same pattern across many decisions: the logical systemic recommendation arrives from the owner, not the agents, because sessions answer the question asked, one turn at a time, and default to agreement over proposal (the disease Rule 3 treats). **Standing countermeasure, all windows:** when a session sees Jorge fighting a CLASS of problem (navigation, credentials, filing, window confusion, token cost), it must volunteer the systemic fix — the automation, removal, or restructure — not just answer the instance in front of it. One proactive systemic recommendation, stated with its tradeoff in one line, is part of doing the job; waiting for the owner to invent it is this RI recurring. (Family: RI-003 upward delegation, RI-043 owner over-involvement — this is the third face of the same failure: the agent behaving like a tool when the owner needed a partner.)

---
**RI-045 · 2026-09-16 — 1Password offering a 6-8 item login dropdown instead of one correct entry, on more than one site (accounts.miamidade.gov, Microsoft/Outlook sign-in).** TRK-2026-9752 (1Password readability/dedup job) was already issued 2026-08-27 to fix exactly this class of clutter vault-wide; its outcome was never verified before this session ended, so today's recurrence may be the SAME unresolved job, not a new failure. **Standing note for the next session that touches this:** before proposing anything, check whether TRK-2026-9752 actually ran (a RESULT file in VTES-Outbox, not just the ISSUED task) — if it never ran, this is a stalled dispatch, not a fix that didn't hold, and the fix is to chase execution, not redesign the protocol. If it DID run and the clutter came back, that's a genuine recurrence and the next fix must rank Tier 1 (re-run the dedup) vs Tier 2 (turn off Chrome's competing built-in password manager so only 1Password ever offers to save/fill, removing the source of re-duplication) vs Tier 3 (a scheduled CLI dedup sweep, e.g. weekly, so clutter never accumulates past a few days) per Rule 4.
