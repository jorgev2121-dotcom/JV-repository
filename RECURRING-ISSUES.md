...
---
**RI-044 recurrence · 2026-09-23 — RAMBO went silent again, this time as a Remote Control session ending.** The "Desktop code" session (session_016fvct…) was live 2026-09-22 18:29 UTC and was archived 2026-09-23 00:52 UTC. Its last state was *waiting on Jorge*: "DBPR blocked by sign-in popup; close the popup and say retry". About 3 hours later the PC was still awake and the VTES poller was ACKing jobs (03:20–04:10 UTC), but no executor was running. Four owner-directive jobs (9952d/e/g/j) sat unrun. **What archived it is not yet known** (window closed, crash, app update, or an idle end). The desktop can read its own logs to find out. **Common thread with every earlier RI-044 case: the executor only runs while a human-opened session is alive, and nothing pages when that session ends.** Rule-4 options were given to Jorge 2026-09-23. Recommended: Tier 2, where the poller launches a fresh headless run per job (JOB-0079 §D.1, never built), with 9952j auto-start as the bridge until then.

---
**RI-044 follow-up · 2026-09-24 — cloud's own diagnosis was wrong, and so was its watch.** On 09-23 the cloud session told Jorge "the PC can't start work by itself" and "Rambo is unreachable." Both were false. `CU-Inbox-Job-Watcher` (JOB-0079 §D) was already launching headless runs; its FAILED-VERIFICATION for 9952d was stamped 03:55 UTC 09-23, *during* that conversation. Remote Control `Jorge-PC` was live, guarded by `CU-ClaudeRemote-Guard`. Cloud couldn't message it, but Jorge could have from the iPhone app. The hourly watch then searched only `RESULT*` titles and missed BLOCKER/FAILED files for ~30 hours. **Lesson (Tier 2, removal):** before telling Jorge something doesn't exist, read `REMOTE-CONTROL-STATUS.md`, the Outbox for any file matching the TRK (not a filename prefix), and `FAILED-VERIFICATION_*`. "I can't reach it" is not "it's unreachable."
