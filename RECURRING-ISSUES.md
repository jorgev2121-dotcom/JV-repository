...
- **RI-032 family (2026-09-04):** 1Password has NO usable access for a portal the owner actively uses — the T&G / CINC HOA portal (tgmgmt.cincwebaxis.com). Owner locked out of autofill again; had to log in without it. Systemic cause candidate: logins created during registration but never saved to the vault (the portal-registration skill mandates the save but it was not enforced/verified). Routed to RAMBO to diagnose (missing item / wrong item / app-not-signed-in / extension-broken) then fix. Durable path pending diagnosis: Tier-2 = make vault-save a mandatory verified registration step + one-time backfill sweep; Tier-3 = Conductor flags any portal lacking a matching 1Password item. Not a payment blocker ($180 is bank ACH). #RI-032 #1password #portal-access
