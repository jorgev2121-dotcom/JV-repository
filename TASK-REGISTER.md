...
| 09-13 | Jorge | **Einar Suarez — Broward Impact-Windows recert/legalization — electrical SUBPERMIT.** Read electrical plan → mark fee-schedule rows → total; reference MASTER permit #; determine owner-vs-contractor signature + whether elec sub needs own NOC or is covered by GC's NOC. Broward municipality TBD (owner). Submit/pay/sign = RED. | RAMBO (prep) + Jorge (municipality + RED submit) | ORDERED | HANDOFF_…einar-broward-electrical-subpermit |
...
| 09-13 | Jorge | **"Expert bot" for county/permit domain knowledge** — owner wants a specialist that holds all the details (microfilm years, form limits, fee schedules, NOC rules) he can't memorize. Cloud proposal: build a `permit-expert` SKILL (like tax-jacket) so any seat has it cold — the charter-aligned form of "expert bot." | Cloud (propose) + owner (approve) | PROPOSED — awaiting owner yes | (this reply) |
