...
| 09-12 | Jorge | **Entity cross-reference layer** — tag every sender/recipient/CC + phone# to the job's TRK; resolve one-person-many-identities (Javi Vasquez case); PEOPLE-REGISTER.csv; owner confirms merges. **Status: ~1 of 5 pieces — doc-level name/email tagging exists in .TAGS.txt; email-participant extraction, phone index, the people register, and entity resolution are NOT built.** | RAMBO (build) + Cloud (email side) | SPEC WRITTEN — mostly unbuilt | OCR-PROTOCOL.md §cross-reference |
