...
| 09-13 | Jorge | **Paid reports/microfilm sitting in inbox un-OCR'd** — all paid (incl. $44.94 8/21 Team USA card), delivered by email, never run through OCR/orphan protocol → read as missing. RAMBO to sweep inbox, OCR+stamp each, match to capsule + to the charge, report denominator; filing = RED. Auto-intake for county attachments = recurrence fix. | RAMBO (sweep) | ORDERED | HANDOFF_…inbox-paid-reports-OCR-sweep |
